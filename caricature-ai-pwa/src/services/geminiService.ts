...
export const validateApiKey = async (settings: AppSettings): Promise<boolean> => {
  try {
    if (settings.provider === "pollinations") {
      ...
      await ai.models.generateContent({ model: "gemini-2.5-flash", contents: "test" as any });
      return true;
      ...
      try {
        const res = await fetch(
          "https://api-inference.huggingface.co/status/timbrooks/instruct-pix2pix",
          {
            headers: { Authorization: `Bearer ${settings.apiKey}` },
          }
        );
...
const getStylePrompt = (style: ArtStyle): string => {
  switch (style) {
    ...
    default:
      // ВАЖНО: для enum используем String(style)
